...
function executeYtDlp(args: string[]): Promise<{ stdout: string; stderr: string; code: number | null }> {
  return new Promise((resolve, reject) => {
    const process = spawn(YT_DLP_PATH, args, { timeout: 300000 })
    let stdout = ""
    let stderr = ""
...
export async function POST(request: NextRequest) {
  try {
    ...
    const outputPath = path.join(OUTPUT_DIR, `${timestamp}.%(ext)s`)
    const expectedOutput = path.join(OUTPUT_DIR, `${timestamp}.mp3`)
    ...
    const videoTitle = stdout.trim() || `audio_${timestamp}`
    const fileName = `${videoTitle}.mp3`
    const downloadUrl = `/downloads/${timestamp}.mp3`

    return NextResponse.json({ success: true, fileName, downloadUrl })
